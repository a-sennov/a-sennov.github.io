...
var codeFontSize  = 105.1316178*1.25 / measureFontSize(codeFontStack) + 'px';

var BODY_STYLESHEET = entag('style', 
    "@import url('https://fonts.googleapis.com/css?family=Oswald|Lora');" +
    ...
    'color:#222;' +
    'font-family:Lora,"Times New Roman",serif; font-size:20px;}');
...
var STYLESHEET = entag('style',
    ...
    // makes "//" look better.
    '.md a{font-family:Lora,\'Times New Roman\'}' +
...
function markdeepToHTML(str, elementMode) {
    ...
    str = str.rp(/(^|[^!])\[([^\[\]]+?)\]\(("?)([^<>\s"]+?)\3(\s+[^\)]*?)?\)/g, function (match, pre, text, maybeQuote, url, attribs) {
        ...
        if(url.lastIndexOf(".") != -1) {
          ...
          return pre + '<a ' + protect('href="' + url + '.md.html"' + attribs) + '>' + text + '</a>' + maybeShowLabel(url);
        }
    });
